...
public class MainServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		/**
		 *  쿠기(Cookie) : 서버가 클라이언트에 다양한 데이터를 저장하기 위해 사용하는 작은 정보 조각
		 ...
		 *				  - 웹 페이지 테마를 어떠한 테마로 설정했느냐 등
		 *
		 *	세션(Session)
		 *		- 쿠키 처럼 서버가 클라이언트에 세션 정보를 저장을 하나 이 세션 정보는 암호화가 된 세션ID이고
		 *		  이 세션 ID에는 어떠한 개인정보도 포함되어 있지 않다.
		 *		- 이 세션 ID가 서버에 전달이 되면 서버에 저장되어있는 동일한 세션 ID의 정보를 획득하여 이전
		 *		  사용자에 대한 기록을 확인 할 수 있다.
		 */
