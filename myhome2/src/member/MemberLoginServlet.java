...
//		request.setAttribute("username", "");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// member_t 테이블에 저장되어있는 username, password(암호화)가 있는지 확인하는 절차
		...
		if(member.getUserid() != null) { // 계정 정보가 있는 경우
			...
			// 기억하기 체크박스가 체크되어있는 경우 아이디 정보를 저장하는 쿠키 생성
			if(remember != null) {
				Cookie cookie = new Cookie("username", username);
				cookie.setMaxAge(60*60*24);	// 만료시간 24 시간으로 설정
				response.addCookie(cookie);
			} else {	// 기억하기 체크박스가 해제되어 있는 경우 기존 아이디 정보를 저장한 쿠키만료시간을 0으로 설정
				for(Cookie c: cookies) {
					if(c.getName().equals("username")) {
						c.setMaxAge(0);
